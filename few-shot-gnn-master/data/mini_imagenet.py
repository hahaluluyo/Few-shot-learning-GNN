...
class MiniImagenet(data.Dataset):
    ...
    def load_dataset(self, partition, size=(84, 84)):
        ...
        if partition == 'train_val':
            ...
            data.update(data_val) # 字典update() 函数，dict.update(dict2)把字典dict2的键/值对更新(添加）到dict里
            del data_val
        else:
        ...
        with open(os.path.join(self.root, 'compacted_datasets', 'mini_imagenet_label_encoder.pickle'),#哪来的lable_encoder.pickle文件啊
                  'rb') as handle:
        ...
        for class_ in data:
            for i in range(len(data[class_])):
                ...
                # Normalize
                image_resized = np.transpose(image_resized, (2, 0, 1))#transpose转置函数，#如果不改变原矩阵，那么正常的参数顺序是：(0,1,2)
