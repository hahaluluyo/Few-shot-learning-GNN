...
class Generator(data.Dataset):
    def __init__(self, root, args, partition='train', dataset='omniglot'):
        ...
        assert (dataset == 'omniglot' or
                dataset == 'mini_imagenet'), 'Incorrect dataset partition' #assert()断言函数，条件为真则通过，否则会报错
        self.dataset = dataset

        if self.dataset == 'omniglot':#黑白图片，单通道就可以
    ...
    def get_task_batch(self, batch_size=5, n_way=20, num_shots=1, unlabeled_extra=0, cuda=False, variable=False):
        ...
        labels_x_global = np.zeros(batch_size, dtype='int64')#用法：zeros(shape, dtype=float, order='C') 返回：返回来一个给定形状和类型的用0填充的数组；
